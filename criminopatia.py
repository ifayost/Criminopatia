...
# Folder Structure
SAVE_PATH = Path(os.getenv('CRIMINOPATIA_SAVE_PATH'))
...
def download_episode(s, article, link):
    ...
    if iframe:
        ...
        # Megaphone Player
        if link[:32] == 'https://playlist.megaphone.fm?e=':
            link = f'https://player.megaphone.fm/playlist/episode/{link[32:]}'
            r = s.get(link)
            data = json.loads(r.text)
            audio_link = data['episodes'][0]['episodeUrlHRef']
            description = data['episodes'][0]['summary']
...
if __name__ == '__main__':
    ...
    username = os.getenv('CRIMINOPATIA_USER')
    password = os.getenv('CRIMINOPATIA_PASSWORD')
